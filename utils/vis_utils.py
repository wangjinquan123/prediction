...
class ArgoMapVisualizer:
    ...
    def show_surrounding_elements(self,
        ...
        # ax.set_xlim(-map_size[0] / 2, map_size[0] / 2)  # 以自车为中心
        ...
        for idx, lane_cl in surrounding_lanes.items():
            # 平移和旋转车道中心线
            # transformed_cl = self.transform_coordinates(lane_cl, position, rot)
            lane_polygon = self.argo_map.get_lane_segment_polygon(
                idx, city_name)
            # transformed_polygon = self.transform_coordinates(lane_polygon[:, 0:2], position, rot)
            ...
            # pt = transformed_cl[0]
            # vec = transformed_cl[1] - transformed_cl[0]
            ax.arrow(pt[0],  #
                     pt[1],
                     vec[0],
                     vec[1],
            ...
            if show_lane_ids:
                m_pt = lane_cl[int(lane_cl.shape[0] / 2)]
                # m_pt = transformed_cl[int(transformed_cl.shape[0] / 2)]
                ax.text(m_pt[0], m_pt[1], idx, color='b')
        ...
        if show_freespace:
            ...
            surrounding_contours = []
            for contour in drivable_area:
                # transformed_contour = self.transform_coordinates(contour[:, 0:2], position, rot)
                # surrounding_contours.append(transformed_contour)
                if (np.min(contour[:, 0]) < x_max
                        and np.min(contour[:, 1]) < y_max
                        and np.max(contour[:, 0]) > x_min
                        and np.max(contour[:, 1]) > y_min):
    ...
    def transform_coordinates(self, coords, position, rot):
        """
        对坐标进行平移和旋转变换，使自车位于中心并坐标系对齐。

        Args:
            coords (np.ndarray): 原始坐标数组，形状为 (N, 2)。
            position (np.ndarray): 自车位置，形状为 (2,)。
            rot (torch.Tensor): 旋转矩阵，形状为 (2, 2)。

        Returns:
            np.ndarray: 变换后的坐标数组，形状为 (N, 2)。
        """
        ...
        if isinstance(position, torch.Tensor):
            ...
            # 平移
        translated = coords - position_np.reshape(1, 2)

        # 旋转
        rotated = translated.dot(rot_np.T)
