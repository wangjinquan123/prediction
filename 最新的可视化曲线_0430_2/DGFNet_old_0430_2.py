...
class DGFNet(nn.Module):
    ...
    def __init__(self, cfg, device):
        ...
        self.ac_am_fusion_rpe = FusionNet_(device=self.device,
                                    config=cfg)

        # self.ac_am_fusion_rpe_1 = FusionNet_1(device=self.device,
        #                                    config=cfg)
    ...
    def forward(self, data, data_ori): # tuple:7
        ...
        # print('actors_ac:',actors_ac) # tensor([[0.0000, 0.0000, 0.7010,  ..., 0.0000, 0.0000, 0.1315],
        lanes_ac = self.lane_net_ac(lanes_ac) # (197,128) simpl

        # ac feature fusion
        actors_ac_1, _ , _ = self.ac_am_fusion(actors_ac, actor_idcs, lanes_ac, lane_idcs, rpe) # (15,128) simpl
        # print("actors_ac_1:",actors_ac_1) # tensor([[ 0.0605, -0.5038, -0.2079,  ..., -0.4931, -0.8101,  1.5860],

        # sc actors/lanes encoding
        actors_sc = self.actor_net_sc(actors_sc) # (15,128)
        lanes_sc = self.lane_net_sc(lanes_sc) # (197,128)

        actors_ac_rpe, _, _ = self.ac_am_fusion_rpe(actors_ac, actor_idcs, rpe) # 001
        # print('actors_ac_rpe1:',actors_ac_rpe) # tensor([[ 0.6580, -0.0634,  0.0629,  ..., -0.0720,  1.7767,  0.4625],

        # actors_ac_rpe, _ ,actors_rpe = self.ac_am_fusion_rpe_1(actors_ac, actor_idcs, rpe) # (15,128) 0405 如果用actor_ac没用的话，再试试这个 002
        # print("actors_ac_rpe2:",actors_ac_rpe) # tensor([[-0.5665,  0.7282, -1.8182,  ..., -0.4308,  0.6243, -0.0238],
        # print('actors_rpe:', actors_rpe) # tensor([[7.6338e-02, 5.6958e-01, 3.7821e-01,  ..., 3.0221e-03, 2.0253e-01,
        ...
        actors_batch_ac_rpe = torch.zeros(batch_size, max_agent_num, self.d_embed, device=self.device) # (1,15,128) 0406
        ...
        for i, actor_ids in enumerate(actor_idcs):
            ...
            actors_batch_ac[i, :num_agents] = actors_ac_1[actor_ids[0] : actor_ids[-1] + 1]

            actors_batch_ac_rpe[i, :num_agents] = actors_ac_rpe[actor_ids[0] : actor_ids[-1] + 1] # 0406
            # print("actors_batch_ac_rpe",actors_batch_ac_rpe) # tensor([[[nan, nan, nan,  ..., nan, nan, nan],
        ...
        topo_pred = self.apply_topo_reasoning(actors_batch_ac_rpe, actors_batch_ac_rpe, self.fuse_layer, self.decoder_layer) # (2,42,42,1) 以为直接输出的是概率  这个拓扑预测还需要斟酌
        ...
        topo_pred_mask = topo_pred_ > 0.5 # 0407 0417_2 概率的判断必须要先归一化 # 178 需根据F1.score来判断阈值  具体取多少阈值仍需商榷
...
class Spatial_Feature_Layer(nn.Module):
    ...
    def forward(self,
        ...
        '''
        # update node
        x, edge, memory = self._build_memory(node, edge)  # (1,140,128) (140,140,128) (140,140,128) +1 +1 +1
        # print('x1', x)  # tensor([[[0.0000, 0.0000, 0.1322,  ..., 0.0000, 0.0000, 1.0819],
        # print('edge', edge)  # tensor([[[ 7.7151e-01,  1.6195e+00,  1.2394e+00,  ...,  1.3960e+00,
        # print('memory', memory)  # tensor([[[0.0000, 0.9228, 0.2512,  ..., 1.3876, 0.0000, 1.1282],
        x_prime, _ = self._mha_block(x, memory, attn_mask=None, key_padding_mask=edge_mask)  # (1,140,128) +1 +1 +1
        # print('x_prime', x_prime)  # tensor([[[ 0.4089,  0.3337,  0.2972,  ..., -0.0837,  0.3004,  0.0124],
        x = self.norm2(x + x_prime).squeeze() # (140,128) +1 +1 +1
        x = self.norm3(x + self._ff_block(x)) # (140,128) +1 +1 +1
        return x, edge, None
...
class SymmetricFusionTransformer(nn.Module):
    ...
    def forward(self, x: Tensor, edge: Tensor, edge_mask: Tensor) -> Tensor:
        ...
        for mod in self.fusion:
            ...
            # print('x',x) # tensor([[-0.4638, -0.2679, -0.1329,  ..., -0.8608, -1.0933,  0.9899],
        return x, None
...
class FusionNet(nn.Module):
    ...
    def forward(self,
        ...
        # projection
        actors = self.proj_actor(actors)  # (42,128)
        print('actors1:',actors) # tensor([[0.0000, 0.0000, 0.2502,  ..., 0.0000, 0.0000, 1.1377],
        lanes = self.proj_lane(lanes)  # (197,128)

        actors_new, lanes_new, actors_new_ = list(), list(), list()
        for a_idcs, l_idcs, rpes in zip(actor_idcs, lane_idcs, rpe_prep): # (15,) (197,) dict:2
            ...
            # actors_ = _actors + rpe[:len(a_idcs)] # new
            print("rpe1:",rpe) # tensor([[[1.4250, 0.0000, 0.0000,  ..., 1.9236, 0.0000, 0.1095],
            # print("rpes['scene_mask']",rpes['scene_mask']) # None
            ...
            # actors_new_.append(actors_) # new
        ...
        # actors_ = torch.cat(actors_new_, dim=0) # new
        # print('actors: ', actors.shape)
        # print('lanes: ', lanes.shape)
        # print("actors_:",actors_)
        return actors, lanes, None #actors_


class Spatial_Feature_Layer_(nn.Module):
    def __init__(self,
                 device,
                 d_edge: int = 128,
                 d_model: int = 128,
                 ...
                 dropout: float = 0.1,
                 update_edge: bool = True) -> None:
        super(Spatial_Feature_Layer_, self).__init__()
        self.device = device
        self.update_edge = update_edge
        ...
        self.multihead_attn_ = MultiheadAttention(
            embed_dim=d_model, num_heads=n_head, dropout=dropout, batch_first=False)
    ...
    def forward(self,
        ...
        '''
        # update node
        x, edge, memory = self._build_memory(node, edge)  # (1,6,128) (6,6,128) (6,6,128) +1 +1 +1
        # print('x1',x) # tensor([[[1.4819e+00, 2.8063e-01, 0.0000e+00, 0.0000e+00, 0.0000e+00,
        # print('edge',edge) # tensor([[[-9.7059e-01, -2.2584e-01, -5.3164e-01,  ..., -7.2914e-01,
        # print('memory',memory) # tensor([[[0.0000, 0.0000, 0.8769,  ..., 1.0205, 1.8482, 1.0586],
        x_prime, _ = self._mha_block(x, memory, attn_mask=None, key_padding_mask=edge_mask)  # (1,6,128) +1 +1 +1
        # print('x_prime',x_prime) # tensor([[[nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan,
    ...
    def _build_memory(self,
        ...
        # 构建内存（无需处理车道）
        # src_x = node.unsqueeze(0).expand(B, B, -1)  # (B, B, d_model)
        # tar_x = node.unsqueeze(1).expand(B, B, -1)  # (B, B, d_model)
    ...
    def _mha_block(self,
        ...
        '''
            ...
            output:
                ...
                :param      [N, N]
        '''
        x, _ = self.multihead_attn_(x, mem, mem,
                                   attn_mask=attn_mask,
                                   key_padding_mask=key_padding_mask,
                                   need_weights=False)  # return average attention weights
        # print('x_mha:',x) # tensor([[[-1.9607e-01,  8.4095e-03,  8.0330e-02, -8.3425e-02,  3.2061e-01, (1,6,128)
        return self.dropout2(x), None
...
class SymmetricFusionTransformer_(nn.Module):
    def __init__(self,
                 device,
                 d_model: int = 128,
                 d_edge: int = 128,
                 ...
                 dropout: float = 0.1,
                 update_edge: bool = True):
        super(SymmetricFusionTransformer_, self).__init__()
        self.device = device

        fusion_ = []
        for i in range(n_layer):
            need_update_edge = False if i == n_layer - 1 else update_edge
            fusion_.append(Spatial_Feature_Layer_(device=device,
                                   d_edge=d_edge,
                                   d_model=d_model,
                                   d_ffn=d_model*2,
                                   n_head=n_head,
                                   dropout=dropout,
                                   update_edge=need_update_edge))
        self.fusion_ = nn.ModuleList(fusion_)
    ...
    def forward(self, x: Tensor, edge: Tensor, edge_mask: Tensor) -> Tensor:
        ...
        '''
        # attn_multilayer = []
        for mod in self.fusion_:
            x, edge, _ = mod(x, edge, edge_mask) # (6,128) (6,6,128) None
            # attn_multilayer.append(attn)
            # print('x',x) # tensor([[nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan, nan,
        return x, None


class FusionNet_(nn.Module):
    def __init__(self, device, config):
        super(FusionNet_, self).__init__()
        self.device = device
        ...
        self.fuse_scene_ = SymmetricFusionTransformer_(self.device,
                                                     d_model=d_embed,
                                                     d_edge=config['d_rpe'],
                                                     n_head=config['n_scene_head'],
                                                     n_layer=config['n_scene_layer'],
    ...
    def forward(self,
        ...
        # print('actors2:',actors) # tensor([[1.5239, 0.0000, 0.1763,  ..., 0.0000, 1.6614, 0.0000],
        # lanes = self.proj_lane(lanes)  # (197,128)

        actors_new = list()
        for a_idcs, rpes in zip(actor_idcs, rpe_prep): # (15,) (197,) dict:2
            # * fusion - scene
            _actors = actors[a_idcs]  # (25,128)
            # _lanes = lanes[l_idcs]  # (197,128)
            # tokens = torch.cat([_actors, _lanes], dim=0)  # (N, d_model)  (212,128)
            num_agents = len(a_idcs)
            rpe = self.proj_rpe_scene(rpes['scene'].permute(1, 2, 0))  # (258,258,128)
            # actors_ = _actors + rpe[:len(a_idcs)] # new brother
            # print("rpe2:",rpe) # tensor([[[0.0000, 0.0000, 0.2726,  ..., 0.0000, 0.0000, 0.0000],

            rpe = rpe[:num_agents, :num_agents, :]  # (25,25,128)
            # rpe_mask = rpes['scene_mask'][:num_agents, :num_agents]  # (B, B)

            # 处理可能的 None 值：动态生成全1掩码
            # if rpes.get('scene_mask') is None: # ?
            #     # 生成全1掩码（所有智能体可见）
            #     rpe_mask = torch.ones(
            #         num_agents, num_agents,
            #         dtype=torch.bool,
            #         device=self.device  # 确保设备一致（GPU/CPU）
            #     )
            # else:
            #     # 截取原掩码的智能体部分
            #     rpe_mask = rpes['scene_mask'][:num_agents, :num_agents]

            # out, _ = self.fuse_scene(tokens, rpe, rpes['scene_mask']) # (212,128)
            out_, _ = self.fuse_scene_(_actors, rpe, rpes['scene_mask'])  # (B, d_embed)

            # actors_new.append(out[:len(a_idcs)]) # list:1
            # lanes_new.append(out[len(a_idcs):]) # list:1
            # actors_new_.append(actors_) # new brother
            actors_new.append(out_)

        # print('actors: ', [x.shape for x in actors_new])
        # print('lanes: ', [x.shape for x in lanes_new])
        actors = torch.cat(actors_new, dim=0) # (15,128)
        # lanes = torch.cat(lanes_new, dim=0) # (197,128)
        # actors_ = torch.cat(actors_new_, dim=0) # new brother
        # print('actors: ', actors) # tensor([[nan, nan, nan,  ..., nan, nan, nan],
        # print('lanes: ', lanes.shape)
        # print("actors_:",actors_)
        return actors, None, None #actors_


# class Spatial_Feature_Layer_1(nn.Module):
#     def __init__(self,
#                  device,
#                  d_edge: int = 128,
#                  d_model: int = 128,
...
#                  dropout: float = 0.1,
#                  update_edge: bool = True) -> None:
#         super(Spatial_Feature_Layer_1, self).__init__()
#         self.device = device
#         self.update_edge = update_edge
#
#         self.proj_memory = nn.Sequential(
...
#
#
# class SymmetricFusionTransformer_1(nn.Module):
#     def __init__(self,
#                  device,
#                  d_model: int = 128,
#                  d_edge: int = 128,
...
#                  dropout: float = 0.1,
#                  update_edge: bool = True):
#         super(SymmetricFusionTransformer_1, self).__init__()
#         self.device = device
#
#         fusion = []
#         for i in range(n_layer):
#             need_update_edge = False if i == n_layer - 1 else update_edge
#             fusion.append(Spatial_Feature_Layer_1(device=device,
#                                    d_edge=d_edge,
#                                    d_model=d_model,
#                                    d_ffn=d_model*2,
#                                    n_head=n_head,
...
#         )
#
#         self.fuse_scene_1 = SymmetricFusionTransformer_1(self.device,
#                                                      d_model=d_embed,
#                                                      d_edge=config['d_rpe'],
#                                                      n_head=config['n_scene_head'],
#                                                      n_layer=config['n_scene_layer'],
...
#         # projection
#         actors = self.proj_actor(actors)  # (15,128)
#         print('actors3:',actors) # tensor([[0.0000, 0.0000, 0.0000,  ..., 0.0000, 0.0000, 0.0984],
#         # lanes = self.proj_lane(lanes)  # (197,128)
#
#         actors_new, actors_new_ = list(), list()
#         for a_idcs, rpes in zip(actor_idcs, rpe_prep): # (15,) (197,) dict:2
#             # * fusion - scene
#             _actors = actors[a_idcs]  # (15,128)
#             # _lanes = lanes[l_idcs]  # (197,128)
...
#
#             # 获取适当大小的rpe
#             # 假设rpe的形状为(N, N, d_rpe)，我们需要截取与actors相关的部分
#             actor_count = len(a_idcs)
#             full_rpe = self.proj_rpe_scene(rpes['scene'].permute(1, 2, 0))  # (258,258,128)
#             # actors_ = _actors + rpe[:len(a_idcs)] # new
#             print("rpe3:",full_rpe) # tensor([[[0.0156, 0.9719, 0.7170,  ..., 0.0000, 0.2994, 0.0000],
#             # print("rpes['scene_mask']",rpes['scene_mask']) # None
#
#             # 截取与actors相关的部分
#             # 这里假设rpe的前actor_count行和列对应于actors
#             # 确保我们只获取与actors相关的RPE部分
#             rpe_for_actors = full_rpe[:actor_count, :actor_count, :]
#
#             # 执行RPE变换 - 这里要确保维度兼容
#             # 如果rpe_for_actors的形状是(actor_count, actor_count, d_embed)
#             # 而我们需要的是(actor_count, d_embed)形式，则需要处理一下
#             actors_ = _actors
#             if rpe_for_actors.size(0) == actor_count and rpe_for_actors.size(1) == actor_count:
#                 # 方案1: 对RPE进行平均，得到每个actor的编码
#                 rpe_actor_encoding = torch.mean(rpe_for_actors, dim=1)  # (actor_count, d_embed)
#                 actors_ = _actors + rpe_actor_encoding
#
#             # 使用SymmetricFusionTransformer进行处理
#             # 这里我们需要确保edge_mask也被相应截取
#             scene_mask = None
#             if 'scene_mask' in rpes and rpes['scene_mask'] is not None:
#                 scene_mask = rpes['scene_mask'][:actor_count, :actor_count]
#
#             out, _ = self.fuse_scene_1(_actors, rpe_for_actors, scene_mask) # (212,128)
#
#             actors_new.append(out[:len(a_idcs)]) # list:1
#             # lanes_new.append(out[len(a_idcs):]) # list:1
#             actors_new_.append(actors_) # new # 确保这里添加了数据！
#
#         # 确认列表非空再进行连接
#         if actors_new:
...
#         else:
#             # 如果列表为空，返回原始actors
#             print("Warning: actors_new is empty!")
#
#         if actors_new_:
#             actors_ = torch.cat(actors_new_, dim=0)
#             # print('actors_:',actors_) # tensor([[1.0700e+00, 5.6958e-01, 3.7821e-01,  ..., 3.0221e-03, 2.0253e-01, (42,128)
#         else:
#             # 如果列表为空，返回原始actors
#             print("Warning: actors_new_ is empty!")
#
#         # print('actors: ', [x.shape for x in actors_new])
#         # print('lanes: ', [x.shape for x in lanes_new])
#         # actors = torch.cat(actors_new, dim=0) # (15,128)
