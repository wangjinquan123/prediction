...
def main():
    ...
    dl_train = DataLoader(train_set,  # 205942
    ...
    dl_val = DataLoader(val_set,  # 2467
    ...
    for epoch in range(args.train_epoches):
        ...
        for i, data in enumerate(tqdm(dl_train, disable=args.no_pbar, ncols=80)):
            data_in = net.pre_process(data) # tuple:7
            out = net(data_in, data) # list:3
            loss_out = loss_fn(out, data, epoch) # dict:4

            post_out = net.post_process(out) # dict:3
            eval_out = evaluator.evaluate(post_out, data) # dict:8
