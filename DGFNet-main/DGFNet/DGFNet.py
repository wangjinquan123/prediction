...
class DGFNet(nn.Module):
    ...
    def forward(self, data, data_ori): # tuple:7
        ...
        actors_batch_sc = torch.zeros(batch_size, max_agent_num, self.d_embed, device=self.device) # (1,15,128)
        actors_batch_ac = torch.zeros(batch_size, max_agent_num, self.d_embed, device=self.device) # (1,15,128)
        ...
        braids = generate_behavior_braids(actors_batch_fu, actors_batch_fu, actors_batch_fu_mask, actors_batch_fu_mask,1)  # (1,14,14,1)
        ...
        topo_pred_mask = F.softmax(topo_pred_mask, dim=-1) # (2,14,14) 0407
        ...
        # print("mask",masks)
        masks[-4] = topo_pred_mask.int()  # //0407 新加 (1,14,14,1) (2,42,42)
        # print("mask[-4]",masks)
        ...
        out_sc = [res_cls_final, res_reg_final, res_reg_fe, topo_pred, braids, actor_topo_mask ] # list:3
        # 看betopNet怎么预测label的

        return out_sc # , history_matrics, braids # list:3
...
def build_topo_layer(self, d_model, actor_d_model, dropout=0.1, num_decoder_layers=1):
    self.actor_topo_fusers = nn.ModuleList(
        [TopoFuser(actor_d_model, actor_d_model // 2, dropout) for _ in range(num_decoder_layers)]  # 10
    )
