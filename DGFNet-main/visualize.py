...
def main():
    ...
    if args.mode != 'test':
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(dataloader)): # data:{dict:21}
                # 对数据加载器迭代，显示进度条。
                if args.seq_id == -1:  # 打出22的那张灰色的gt
                    data_in = net.pre_process(data) # tuple:10
                    out, history_matrics, braids = net(data_in, data)
                    ...
                    vis.draw_once(post_out, data, eval_out, history_matrics=history_matrics, braids=braids, show_map=True, split=args.mode)
